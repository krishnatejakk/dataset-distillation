...
import networks
import train_distilled_image_zen
import utils
...
def main(state):
    ...
    if state.mode == 'train':
        ...
        while cur_idx < end_idx:
            ...
            models = networks.get_zen_networks(state, N=(next_cur_idx - cur_idx))
        ...
        def load_train_models():
            if state.train_nets_type == 'unknown_init':
                models = networks.get_zen_networks(state, N=1)
                return [model for model in models for _ in range(state.local_n_nets)]
            elif state.train_nets_type == 'known_init':
                return networks.get_zen_networks(state, N=state.local_n_nets)
            elif state.train_nets_type == 'loaded':
                models = networks.get_zen_networks(state, N=state.local_n_nets)
        ...
        if state.test_nets_type == 'unknown_init':
            test_models = networks.get_zen_networks(state, N=1)
            state.test_models = [test_model for test_model in test_models for _ in range(state.local_test_n_nets)]
            ...
            state.test_models = networks.get_zen_networks(state, N=state.local_test_n_nets)
        ...
        if state.phase == 'train':
            ...
            steps = train_distilled_image_zen.distill(state, state.models)
